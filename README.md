...
<div align="center" style="font-size: 1.5em; margin: 20px 0;">
    <strong>MtsDevSecops</strong> — Inteligência Artificial Autônoma para Testes de Penetração
</div>
...
flowchart TB
    ...
    pentester --> |Usa HTTPS| mtsdevsecops
    pentester --> |Monitora IA HTTPS| langfuse
    pentester --> |Monitora Sistema HTTPS| grafana
    mtsdevsecops --> |Testa Vários protocolos| target
    mtsdevsecops --> |Consulta HTTPS| llm
    mtsdevsecops --> |Busca HTTPS| search
    mtsdevsecops --> |Reporta HTTPS| langfuse
    mtsdevsecops --> |Reporta HTTPS| grafana

    class pentester person
    class mtsdevsecops system
...
```bash
mkdir mtsdevsecops && cd mtsdevsecops
```
...
Acesse [localhost:8443](https://localhost:8443) para acessar a Web UI (padrão: `admin@mtsdevsecops.com` / `admin`)

> **Nota:** É necessário configurar pelo menos um provedor de modelo de linguagem (OpenAI, Anthropic, Gemini, AWS Bedrock ou Ollama) para usar o MtsDevSecops. Chaves de API para buscadores são opcionais mas recomendadas.

### Acesso Externo à Rede

Por padrão, o MtsDevSecops escuta apenas em `127.0.0.1`. Para acesso externo:
...
O MtsDevSecops oferece acesso programático completo via REST e GraphQL APIs.
...
# Via Docker
docker run --rm -v $(pwd)/.env:/opt/mtsdevsecops/.env mtsdevsecops:latest /opt/mtsdevsecops/bin/ctester -verbose
```
...
docker exec -it mtsdevsecops /opt/mtsdevsecops/bin/etester test

# Informações do banco de embeddings
docker exec -it mtsdevsecops /opt/mtsdevsecops/bin/etester info -verbose
```
...
docker build -t mtsdevsecops:dev .
```

## Prompts de Pentest Rápido

Templates prontos para acelerar testes de penetração com o MtsDevSecops.

### Reconhecimento Rápido

```
Execute reconhecimento rápido no alvo {IP/DOMÍNIO}:
1. nmap -T4 --top-ports 1000 -sV -sC --min-rate 1000
2. Identifique serviços e versões expostos
3. Verifique CVEs conhecidos para cada serviço encontrado
Timeout: 60s por ferramenta. Gere relatório parcial imediato.
```

### Pentest Web Rápido

```
Teste o alvo web {URL}:
1. Enumere diretórios com gobuster (wordlist common.txt)
2. Teste credenciais padrão nos painéis encontrados
3. sqlmap --batch --level 1 --risk 1 nos parâmetros GET/POST
4. Teste XSS e command injection nos inputs
Foque OWASP Top 10. Máximo 2 tentativas por vetor antes de pular.
```

### Exploração Direta

```
Explore o serviço {SERVIÇO} v{VERSÃO} no alvo {IP}:{PORTA}:
1. Busque exploits conhecidos no Sploitus e ExploitDB
2. Tente o exploit mais relevante via Metasploit
3. Se obter acesso, tente escalação de privilégios
Se não funcionar em 2 tentativas, passe para próximo vetor.
```

### Scan Completo Otimizado

```
Pentest completo no alvo {IP/DOMÍNIO}:
Fase 1 - Recon: nmap -T4 -p- --min-rate 1000 + detecção de serviços
Fase 2 - Vulns: Scripts NSE + busca de CVEs nos serviços encontrados
Fase 3 - Exploit: Credenciais padrão → SQLi → RCE → Privesc
Execute fases em paralelo quando possível. Relatório ao final de cada fase.
```
